...
class TrajectoryPlanner(Node):
    def __init__(self):
        super().__init__('trajectory_planner_M5')
        self.ego_pose = None
        self.ego_twist = None
        self.path = []
        ...
        self.ready_to_publish_student = False  # gets set True in path_callback
        ...
        # Feedback state (restored from old code)
    ...
    def feedback_callback(self, msg):
        # Restored from old code to use feedback for speed/steering error logs
    ...
    def control_loop(self):
        ...
        if missing:
            if not self.missing_input_warned:
                self.get_logger().warn(f"check your inputs buddy (s): {', '.join(missing)}")
        ...
        self.prune_path()
        if not self.path:
            self.get_logger().warn("All path points are behind the vehicle. Stopping and requesting new path.")
        ...
        # --- Steering and speed logic (now includes feedback-based error logs) ---
        ...
        # Feedback-based error calculation (restored from old code)
        speed_error = abs(self.feedback_speed - target_speed)
        steer_error = abs(self.feedback_steering_angle - steering_angle_deg)
 
        log_msg = (
            f"Target: V={target_speed:.2f}, Steer={steering_angle_deg:.2f} deg | "
            f"Feedback: V={self.feedback_speed:.2f}, Steer={self.feedback_steering_angle:.2f} deg | "
            f"Error: V={speed_error:.2f}, Steer={steer_error:.2f}"
        )
        if saturated:
            log_msg += " [STEERING SATURATED]"
    ...
    def pure_pursuit_steering(self):
        ...
        if x_r <= 0:
            self.get_logger().warn("Lookahead point is behind the vehicle. Stopping and requesting new path.")
            return 0.0, False
        ...
        steering_angle_rad = math.atan(L * curvature)
    ...
    def publish_velocity(self, speed, steering_angle_deg=0.0):
        ...
        if not self.loop_stopped:
            self.get_logger().info(f"[DRIVE] speed: {speed:.2f} m/s | steering_angle: {steering_angle_deg:.2f} deg")
